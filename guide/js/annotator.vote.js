	...
	this.pluginInit = function() {
		this.annotator
			...
			.subscribe("annotationViewerShown", function(viewer, annotations) {
				// strip URLs from the viewer
				var vtext = $('.annotator-annotation span.annotator-controls+div').text()
				vtext = stripUrl(vtext);
				$('.annotator-annotation span.annotator-controls+div').text(vtext);
...
Annotator.Plugin.Vote.prototype.updateViewer = function(field, annotation) {
	...
	for (var i = 0; i < strings.length; i++) {
		if (isImageUrl($.trim(strings[i]))) {
			$(field).append('<div class="annotator-imageViewer"><img src="' + strings[i] + '" width="200"/></div>');
		} 
	}
