...
function setupReviewNotes(annotator, destinationElement) {
	...
	for (var i = 0; i < annotations.length; i++) {
		...
		var notes = $('<div class="vvReviewNote">'+stripUrl(annotation.text)+'</div>');

		var imageContainer = $('<div/>');
		
		var strings = annotation.text.split(/\s+/);
		for (var j = 0; j < strings.length; j++) {
			if (isImageUrl($.trim(strings[j]))) {
				$(imageContainer).append('<div class="annotator-imageViewer"><img src="' + strings[j] + '" width="200"/></div>');
			} 
		}
		...
		note.append(notes);
		if (imageContainer.children().length > 0) note.append(imageContainer);
		note.append(icons);
...
function vvSpeak(text) {
	if (speechAvailable) {
		...
		var speaking = meSpeak.speak(text, { variant: 'f2', speed: 120 });
	} else {
